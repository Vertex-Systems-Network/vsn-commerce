...
docker compose run --rm test php artisan test --configuration=phpunit.postgres.xml
laravelTests=true
echo '[runtime] run live-provider contract tests with faked HTTP'
docker compose run --rm test php artisan test --filter=ProviderIntegrationTest --configuration=phpunit.postgres.xml
