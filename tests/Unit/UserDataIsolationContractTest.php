...
class UserDataIsolationContractTest extends TestCase
{
    ...
    public function test_customer_controllers_keep_personal_records_user_scoped(): void
    {
        ...
        foreach($contracts as $file=>$needles){
            ...
            foreach($needles as $needle) $this->assertStringContainsString($needle,$source,"Missing authenticated-user scope in {$file}");
        }
    }
    ...
    public function test_account_overview_uses_authenticated_server_endpoints(): void
    {
        ...
        $store=file_get_contents(dirname(__DIR__,2).'/resources/js/platform/store.jsx');
        $this->assertStringContainsString('const initialOrders=[];',$store);
        $this->assertStringContainsString('const initialNotifications=[];',$store);
        $this->assertStringContainsString('const initialMessages=[];',$store);
    }
}
