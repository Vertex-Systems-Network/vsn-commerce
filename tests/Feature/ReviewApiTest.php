...
class ReviewApiTest extends TestCase
{
    ...
    public function test_more_than_four_review_images_are_rejected(): void
    {
        ...
        $images = [];
        for ($i = 0; $i < 5; $i++) {
            $images[] = UploadedFile::fake()->image("review-{$i}.jpg");
        }
    ...
    public function test_retrying_same_order_line_does_not_create_duplicate_review_or_coupon(): void
    {
        ...
        $this->assertDatabaseCount('reviews', 1);
        $this->assertDatabaseCount('review_reward_coupons', 1);
    }
    ...
    {
        $user = User::factory()->create(['name'=>'Fatima Rahman']);
        [$product, , , $item] = $this->purchase($user, OrderStatus::Delivered, 0);
        Sanctum::actingAs($user);
        ...
        $this->assertSame(1, (int) $product->fresh()->reviews_count);
        $this->assertSame('4.00', (string) $product->fresh()->rating);
    ...
    public function test_rejected_review_revokes_an_unused_coupon(): void
    {
        ...
        $moderator = User::factory()->create(['role'=>UserRole::Admin]);
        Sanctum::actingAs($moderator);
    ...
    {
        $user = User::factory()->create();
        [, , , $sourceItem] = $this->purchase($user, OrderStatus::Delivered, 0);
        Sanctum::actingAs($user);
        ...
        $checkout = $this->postJson('/api/v1/checkout/sessions', $payload)->assertOk()->assertJsonPath('data.totals.discountMinor', 10_000)->json('data');
        ...
        $checkout = $this->postJson('/api/v1/checkout/sessions', $payload)->assertOk()->json('data');
    ...
    public function test_review_coupon_cannot_be_used_by_another_account(): void
    {
        ...
        [, , , $item] = $this->purchase($owner, OrderStatus::Delivered, 0);
        ...
        [, $address] = $this->activeCart($other, 100_000);
    ...
    public function test_review_reminder_is_queued_only_once_for_eligible_delivered_line(): void
    {
        ...
        [, , $order, $item] = $this->purchase($user, OrderStatus::Delivered, 0);
