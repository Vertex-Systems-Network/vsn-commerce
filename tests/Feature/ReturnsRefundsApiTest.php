...
class ReturnsRefundsApiTest extends TestCase
{
    ...
    {
        [$user,$order,$item] = $this->order('cod', 1, 100_000, 0);
    ...
    {
        [$user,$order,$item,$inventory] = $this->order('cod', 1, 100_000, 0);
        $admin = User::factory()->create(['role' => UserRole::Admin]);
        Sanctum::actingAs($user);
        $ret = $this->postJson('/api/v1/returns', ['orderId' => $order->public_id, 'reason' => 'Wrong item received', 'resolution' => 'refund_original'])->assertOk()->json('data');
    ...
    {
        [$user,$order] = $this->order('cod', 1, 100_000, 0);
        Wallet::create(['user_id' => $user->id, 'balance_coins' => 0, 'reserved_coins' => 0]);
        $admin = User::factory()->create(['role' => UserRole::Admin]);
        Sanctum::actingAs($user);
        $ret = $this->postJson('/api/v1/returns', ['orderId' => $order->public_id, 'reason' => 'Not as described', 'resolution' => 'coins'])->assertOk()->json('data');
        Sanctum::actingAs($admin);
        $this->postJson("/api/v1/admin/returns/{$ret['id']}/review", ['approve' => true])->assertOk();
    ...
    {
        [$user,$order] = $this->order('card', 1, 100_000, 0);
        $this->capturedIntent($user, $order, 100_000);
        $admin = User::factory()->create(['role' => UserRole::Admin]);
        Sanctum::actingAs($user);
        $ret = $this->postJson('/api/v1/returns', ['orderId' => $order->public_id, 'reason' => 'Item damaged', 'resolution' => 'refund_original'])->assertOk()->json('data');
        Sanctum::actingAs($admin);
        $this->postJson("/api/v1/admin/returns/{$ret['id']}/review", ['approve' => true])->assertOk();
        $this->postJson("/api/v1/admin/returns/{$ret['id']}/receive")->assertOk()->assertJsonPath('data.refund.status', 'completed');
    ...
    {
        [$user,$order,$item] = $this->order('cod', 2, 100_000, 1000);
        $beneficiary = User::factory()->create();
        Wallet::create(['user_id' => $beneficiary->id, 'balance_coins' => 7000, 'reserved_coins' => 0]);
        ...
        Sanctum::actingAs($admin);
        $this->postJson("/api/v1/admin/returns/{$ret['id']}/review", ['approve' => true])->assertOk();
        $this->postJson("/api/v1/admin/returns/{$ret['id']}/receive")->assertOk();
        ...
        $this->assertSame(5_000, (int) $vendorOrder->fresh()->platform_commission_reversed_minor);
    ...
    public function test_dispute_requires_admin_outcome_before_return_progresses(): void
    {
        ...
        Sanctum::actingAs($admin);
        $this->postJson("/api/v1/admin/disputes/{$ret['dispute']['id']}/resolve", ['outcome' => 'replacement', 'note' => 'Evidence supports buyer'])->assertOk()->assertJsonPath('data.outcome', 'replacement');
    ...
    {
        [$user,$order,$item] = $this->order('cod', 3, 150_000, 0);
        $admin = User::factory()->create(['role' => UserRole::Admin]);
        Sanctum::actingAs($user);
    ...
    {
        [$user,$order,$item,$inventory] = $this->order('cod', 2, 100_000, 0);
        $admin = User::factory()->create(['role' => UserRole::Admin]);
        Sanctum::actingAs($user);
    ...
    private function order(string $method, int $qty, int $lineTotal, int $commissionBps): array
    {
        ...
        $vendor = Vendor::create(['name' => 'Seller', 'slug' => 'seller-'.Str::lower((string) Str::ulid()), 'status' => 'active', 'commission_bps' => $commissionBps]);
        ...
        $warehouse = Warehouse::create(['code' => 'WH-'.Str::ulid(), 'name' => 'Main']);
        $inventory = Inventory::create(['warehouse_id' => $warehouse->id, 'product_variant_id' => $variant->id, 'on_hand' => 9, 'reserved' => 0, 'safety_stock' => 0]);
