...
class GameApiTest extends TestCase
{
    ...
    public function test_join_debits_wallet_once_and_retry_is_idempotent(): void
    {
        ...
        Wallet::create(['user_id' => $user->id, 'balance_coins' => 1000, 'reserved_coins' => 0]);
    ...
    public function test_game_entry_idempotency_key_cannot_cross_users(): void
    {
        ...
        Sanctum::actingAs($a);
        $this->postJson("/api/v1/games/{$game->public_id}/entries", $payload)->assertCreated();
        Sanctum::actingAs($b);
        $this->postJson("/api/v1/games/{$game->public_id}/entries", $payload)
    ...
    {
        $user = User::factory()->create();
        Wallet::create(['user_id' => $user->id, 'balance_coins' => 100, 'reserved_coins' => 0]);
    ...
    {
        $user = User::factory()->create();
        Wallet::create(['user_id' => $user->id, 'balance_coins' => 500, 'reserved_coins' => 0]);
        $game = $this->makeGame();
    ...
    public function test_admin_can_create_close_draw_and_fulfill_game(): void
    {
        ...
        Wallet::create(['user_id' => $winner->id, 'balance_coins' => 500, 'reserved_coins' => 0]);
