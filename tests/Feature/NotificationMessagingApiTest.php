...
class NotificationMessagingApiTest extends TestCase
{
    ...
    public function test_notification_can_be_read_and_read_all_is_user_scoped(): void
    {
        ...
        $publish = app(PublishMarketplaceNotification::class);
        $n = $publish->execute($a, 'orders', 'a', 'A', 'A body', 'a-1');
        $publish->execute($a, 'orders', 'a2', 'A2', 'A2 body', 'a-2');
        $publish->execute($b, 'orders', 'b', 'B', 'B body', 'b-1');
        Sanctum::actingAs($a);
        $this->postJson("/api/v1/notifications/{$n->public_id}/read")->assertOk()->assertJsonPath('data.read', true);
        $this->postJson('/api/v1/notifications/read-all')->assertOk()->assertJsonPath('data.unreadCount', 0);
    ...
    public function test_email_outbox_dispatches_once(): void
    {
        Mail::spy();
        $user = User::factory()->create();
        $row = app(PublishMarketplaceNotification::class)->execute($user, 'orders', 'mail.test', 'Mail test', 'Body', 'mail-test-1');
        app(DispatchNotificationDeliveries::class)->execute();
        app(DispatchNotificationDeliveries::class)->execute();
    ...
    public function test_customer_support_conversation_and_message_send_are_idempotent(): void
    {
        ...
        $same = $this->postJson('/api/v1/messages/conversations', ['kind' => 'support'])->assertOk()->json('data');
        $this->assertSame($conversation['id'], $same['id']);
    ...
    {
        [$buyer,$owner,$order,$vendorOrder] = $this->sellerOrder();
        $stranger = User::factory()->create();
        Sanctum::actingAs($buyer);
        $conversation = $this->postJson('/api/v1/messages/conversations', ['kind' => 'order', 'vendorOrderId' => $vendorOrder->public_id])->assertOk()->json('data');
        Sanctum::actingAs($owner);
        $this->getJson("/api/v1/messages/conversations/{$conversation['id']}")->assertOk();
        Sanctum::actingAs($stranger);
        $this->getJson("/api/v1/messages/conversations/{$conversation['id']}")->assertNotFound();
    ...
    {
        [$buyer,$owner,,$vendorOrder] = $this->sellerOrder();
        Sanctum::actingAs($buyer);
        $conversation = $this->postJson('/api/v1/messages/conversations', ['kind' => 'order', 'vendorOrderId' => $vendorOrder->public_id])->assertOk()->json('data');
        $this->postJson("/api/v1/messages/conversations/{$conversation['id']}/messages", ['body' => 'Seller question', 'clientId' => 'buyer-msg-1'])->assertOk();
        Sanctum::actingAs($owner);
        $this->getJson('/api/v1/activity')->assertOk()->assertJsonPath('data.messagesUnread', 1);
        $this->getJson("/api/v1/messages/conversations/{$conversation['id']}")->assertOk();
        $this->getJson('/api/v1/activity')->assertOk()->assertJsonPath('data.messagesUnread', 0);
    }
    ...
    public function test_message_attachment_is_private_to_conversation_participants(): void
    {
        ...
        Sanctum::actingAs($owner);
        $this->get("/api/v1/messages/attachments/{$attachmentId}")->assertOk();
        Sanctum::actingAs(User::factory()->create());
        $this->get("/api/v1/messages/attachments/{$attachmentId}")->assertNotFound();
    }
    ...
    public function test_support_agent_can_join_customer_support_thread_and_reply(): void
    {
        ...
        $this->getJson('/api/v1/messages/conversations')->assertOk()->assertJsonPath('data.items.0.id', $conversation['id']);
        ...
        $this->assertDatabaseHas('conversation_participants', ['conversation_id' => Conversation::where('public_id', $conversation['id'])->value('id'), 'user_id' => $support->id, 'participant_role' => 'support']);
    }
    ...
    public function test_chat_message_facts_are_immutable_at_model_layer(): void
    {
        ...
        Sanctum::actingAs($user);
        $conversation = $this->postJson('/api/v1/messages/conversations', ['kind' => 'support'])->assertOk()->json('data');
        $this->postJson("/api/v1/messages/conversations/{$conversation['id']}/messages", ['body' => 'Immutable', 'clientId' => 'immutable-1'])->assertOk();
        $message = ConversationMessage::firstOrFail();
        $this->expectException(\LogicException::class);
        $message->update(['body' => 'tampered']);
    }
    ...
    private function sellerOrder(): array
    {
        ...
        $vendor = Vendor::create(['owner_user_id' => $owner->id, 'name' => 'Message Seller', 'slug' => 'message-seller-'.Str::lower((string) Str::ulid()), 'status' => 'active', 'commission_bps' => 1000]);
