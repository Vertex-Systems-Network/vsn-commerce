...
class ShippingApiTest extends TestCase
{
    ...
    {
        [, $owners,$order]=$this->order();$vo=$order->vendorOrders()->firstOrFail();Sanctum::actingAs($owners[0]);
        ...
        $this->assertSame($a,$b);$this->assertDatabaseCount('shipments',1);
    }
    ...
    public function test_pending_shipment_provider_creation_can_be_retried_without_new_shipment_row(): void
    {
        ...
        $this->assertDatabaseCount('shipments',1);$this->assertGreaterThanOrEqual(2,$shipment->fresh()->creation_attempts);
    }
    ...
    {
        [, $owners,$order]=$this->order();$shipment=$this->createShipment($owners[0],$order);Sanctum::actingAs($owners[0]);
    ...
    {
        [, $owners,$order]=$this->order();$order->update(['payment_method'=>'card','payment_status'=>PaymentStatus::Pending]);$vo=$order->vendorOrders()->firstOrFail();Sanctum::actingAs($owners[0]);
    ...
    {
        [$customer,$owners,$order]=$this->order();$shipment=$this->createShipment($owners[0],$order);
        ...
        Sanctum::actingAs($customer);$this->getJson("/api/v1/shipments/{$shipment->public_id}")->assertOk()->assertJsonPath('data.events.1.location','Lahore Hub');
    }
    ...
    public function test_same_provider_event_id_with_changed_payload_is_rejected_as_replay_mismatch(): void
    {
        ...
        $changed=$first;$changed['status']='delivered';$changed['message']='Changed replay payload';
    ...
    {
        [, $owners,$order]=$this->order();$order->update(['payment_status'=>PaymentStatus::Paid]);$shipment=$this->createShipment($owners[0],$order);
        ...
        $vo=$order->vendorOrders()->first()->fresh();$this->assertNotNull($vo->delivered_at);$this->assertSame('delivered',$order->fresh()->status->value);
    ...
    {
        [, $owners,$order]=$this->order(2);$order->update(['payment_status'=>PaymentStatus::Paid]);
        $shipA=$this->createShipment($owners[0],$order,0);$this->shippingWebhook(['id'=>'multi-a-delivered','shipment_id'=>$shipA->provider_shipment_id,'status'=>'delivered','occurred_at'=>now()->toIso8601String()])->assertOk();
        $this->assertSame('shipped',$order->fresh()->status->value);$this->assertNull($order->fresh()->delivered_at);
        $shipB=$this->createShipment($owners[1],$order,1);$this->shippingWebhook(['id'=>'multi-b-delivered','shipment_id'=>$shipB->provider_shipment_id,'status'=>'delivered','occurred_at'=>now()->addMinute()->toIso8601String()])->assertOk();
        $this->assertSame('delivered',$order->fresh()->status->value);$this->assertNotNull($order->fresh()->delivered_at);
    }
    ...
    {
        [, $owners,$order]=$this->order();$shipment=$this->createShipment($owners[0],$order);$shipment->update(['dispatch_due_at'=>now()->subMinute()]);
        $result=app(CheckShippingSlas::class)->execute();$this->assertSame(1,$result['dispatchBreaches']);$this->assertNotNull($shipment->fresh()->dispatch_breached_at);
        $event=ShipmentEvent::query()->where('shipment_id',$shipment->id)->where('code','sla.dispatch_breached')->firstOrFail();
        $this->expectException(\LogicException::class);$event->update(['message'=>'tampered']);
    }
    ...
    {
        [, $owners,$order]=$this->order(2);$shipment=$this->createShipment($owners[0],$order,0);
        Sanctum::actingAs($owners[1]);$this->postJson("/api/v1/vendor/shipments/{$shipment->public_id}/ready")->assertNotFound();
        $admin=User::factory()->create(['role'=>UserRole::Admin]);Sanctum::actingAs($admin);
    ...
    {
        $vo=$order->vendorOrders()->orderBy('id')->get()[$vendorIndex];Sanctum::actingAs($owner);
    ...
    {
        $customer=User::factory()->create();$address=Address::create(['user_id'=>$customer->id,'label'=>'Home','recipient_name'=>$customer->name,'phone'=>'03001234567','line1'=>'1 Test Street','city'=>'Lahore','state'=>'Punjab','postal_code'=>'54000','country_code'=>'PK','is_default'=>true]);
        $owners=[];$products=[];
        for($i=0;$i<$vendorCount;$i++){
            $owner=User::factory()->create(['role'=>UserRole::Seller]);$owners[]=$owner;
            ...
            Inventory::create(['warehouse_id'=>$warehouse->id,'product_variant_id'=>$variant->id,'on_hand'=>10,'reserved'=>0,'safety_stock'=>0]);$products[]=$product;
        }
        Sanctum::actingAs($customer);foreach($products as $product)$this->postJson('/api/v1/cart/items',['productId'=>$product->id,'quantity'=>1])->assertOk();
