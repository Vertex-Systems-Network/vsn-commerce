...
export function Checkout() {
	...
	useEffect(() => {
		let live = true;
		apiGet("/wallet").then((next) => { if (live) { setWallet(next); setWalletError(""); } }).catch((err) => live && setWalletError(err.message || "Wallet data could not be loaded."));
		return () => { live = false; };
	}, []);
	...
	useEffect(() => {
		...
		apiGet("/addresses")
			...
			.catch((err) => live && setAddressError(err.message || "Sign in to continue to checkout."))
			.finally(() => live && setLoading(false));
		return () => { live = false; };
	}, []);
	...
	const reserveCheckout = async () => {
		...
		try {
			...
			setSession(next);
			apiGet("/wallet").then((nextWallet) => { setWallet(nextWallet); setWalletError(""); }).catch((err) => setWalletError(err.message || "Wallet data could not be refreshed."));
			setPaymentIntent(null);
			setPaymentKey(`payment-${Date.now()}-${Math.random().toString(36).slice(2)}`);
			setStep(4);
	...
	const placeOrder = async () => {
		...
		try {
			...
			await apiGet("/wallet").then((nextWallet) => { setWallet(nextWallet); setWalletError(""); }).catch((err) => setWalletError(err.message || "Wallet data could not be refreshed."));
	...
	const retryPayment = async () => {
		if (!paymentIntent?.id || paymentIntent.status !== "failed") return;
		setBusy(true); setError("");
		try {
			if (paymentIntent.canRetryInitialization) {
				setPaymentIntent(await apiPost(`/payments/${paymentIntent.id}/retry-initialization`, {}));
			} else {
				setPaymentIntent(null);
				setPaymentKey(`payment-${Date.now()}-${Math.random().toString(36).slice(2)}`);
			}
		} catch (err) { setError(err.message || "Payment could not be retried."); }
		finally { setBusy(false); }
	};
	...
	if (addressError && !session) return <FailurePage title="Checkout" sub="Server-authoritative marketplace checkout." error={addressError} />;
	...
	return <Page title="Checkout" sub="Laravel checkout with server pricing, signed payment webhooks, seller-aware shipping and inventory reservation.">
		<div className="checkout-layout"><div>
			...
			{step === 3 && <Card><h2>Payment & adjustments</h2>{walletError && <Status>{walletError} VSN Coins are unavailable; other server payment methods remain available.</Status>}{(options.paymentMethods || []).map((method) => { const enabled = method.enabled && (method.code !== "coins" || hasServerCoinRate); return <label className={`choice-row ${enabled ? "" : "is-disabled"}`} key={method.code} htmlFor={`checkout-payment-${method.code}`} aria-label={`Payment method ${method.name || method.code}`}><input id={`checkout-payment-${method.code}`} type="radio" name="pay" disabled={!enabled} checked={payment === method.code} onChange={() => { if (!enabled) return; setPayment(method.code); if (method.code === "coins") setCoinRedemption(maxRedeemableCoins); setSession(null); setPaymentIntent(null); setPaymentKey(`payment-${Date.now()}-${Math.random().toString(36).slice(2)}`); setAttemptKey(`checkout-${Date.now()}-${Math.random().toString(36).slice(2)}`); }} /><span><b>{method.name}</b><small>{method.code === "coins" && !hasServerCoinRate ? "Coin conversion is unavailable from the server." : method.description}</small></span></label>; })}
				{payment === "card" && <div><h3>Saved payment method</h3>{(options.savedPaymentMethods || []).length ? options.savedPaymentMethods.map((method) => <label className="choice-row" key={method.id} htmlFor={`checkout-saved-card-${method.id}`} aria-label={`Saved payment method ${(method.brand || "Card").toUpperCase()} ending ${method.last4 || ""}`}><input id={`checkout-saved-card-${method.id}`} type="radio" name="saved-card" checked={savedPaymentMethodId === method.id} onChange={() => { setSavedPaymentMethodId(method.id); setSession(null); setPaymentIntent(null); setAttemptKey(`checkout-${Date.now()}-${Math.random().toString(36).slice(2)}`); }} /><span><b>{(method.brand || "Card").toUpperCase()} •••• {method.last4}</b><small>{method.default ? "Default · " : ""}{method.provider} · expires {String(method.expiry?.month || "--").padStart(2,"0")}/{method.expiry?.year || "----"}</small></span></label>) : <p>No saved card yet. <Link to="/profile">Manage payment methods</Link>.</p>}</div>}
