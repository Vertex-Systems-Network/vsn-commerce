...
export function Checkout() {
	...
	return <Page title="Checkout" sub="Laravel checkout with server pricing, signed payment webhooks, seller-aware shipping and inventory reservation.">
		<div className="checkout-layout"><div>
			...
			{step === 3 && <Card><h2>Payment & adjustments</h2>{(options.paymentMethods || []).map((method) => { const enabled = method.enabled && (method.code !== "coins" || hasServerCoinRate); return <label className={`choice-row ${enabled ? "" : "is-disabled"}`} key={method.code}><input type="radio" name="pay" disabled={!enabled} checked={payment === method.code} onChange={() => { if (!enabled) return; setPayment(method.code); if (method.code === "coins") setCoinRedemption(maxRedeemableCoins); setSession(null); setPaymentIntent(null); setPaymentKey(`payment-${Date.now()}-${Math.random().toString(36).slice(2)}`); setAttemptKey(`checkout-${Date.now()}-${Math.random().toString(36).slice(2)}`); }} /><span><b>{method.name}</b><small>{method.code === "coins" && !hasServerCoinRate ? "Coin conversion is unavailable from the server." : method.description}</small></span></label>; })}
				{payment === "card" && <div><h3>Saved payment method</h3>{(options.savedPaymentMethods || []).length ? options.savedPaymentMethods.map((method) => <label className="choice-row" key={method.id}><input type="radio" name="saved-card" checked={savedPaymentMethodId === method.id} onChange={() => { setSavedPaymentMethodId(method.id); setSession(null); setPaymentIntent(null); setAttemptKey(`checkout-${Date.now()}-${Math.random().toString(36).slice(2)}`); }} /><span><b>{(method.brand || "Card").toUpperCase()} •••• {method.last4}</b><small>{method.default ? "Default · " : ""}{method.provider} · expires {String(method.expiry?.month || "--").padStart(2,"0")}/{method.expiry?.year || "----"}</small></span></label>) : <p>No saved card yet. <Link to="/profile">Manage payment methods</Link>.</p>}</div>}
...
export function Notifications() {
	...
	return <Page title="Notifications" sub="Orders, shipping, games, gifts, reviews, returns and account alerts from Laravel.">{error&&<Status>{error}</Status>}<Card><div className="section-title"><div><h3>{meta.unreadCount||0} unread</h3><p>Server-authoritative notification history.</p></div><Button disabled={!meta.unreadCount} onClick={readAll}>Mark all read</Button></div><div className="simple-list">{loading?<p>Loading notifications…</p>:items.length?items.map((item)=><div key={item.id} onClick={()=>markRead(item)} style={{cursor:item.read?"default":"pointer"}}><FaBell/><span><b>{item.title}</b><small>{item.body}</small><small>{item.createdAt?new Date(item.createdAt).toLocaleString():""} · {item.category}</small>{item.actionUrl&&<Link to={item.actionUrl}>View details</Link>}</span><Badge tone={item.read?"neutral":"primary"}>{item.read?"Read":"New"}</Badge></div>):<p>No notifications yet.</p>}</div></Card></Page>;
}
...
export function Messages() {
	...
	const loadThreads=async()=>{try{const data=await apiGet("/messages/conversations");setThreads(data.items||[]);if(!selected&&data.items?.length)setSelected(data.items[0].id);setLoaded(true);}catch(e){setError(e.message||"Messages unavailable.");setLoaded(true);}};
	const loadConversation=async(id=selected)=>{if(!id)return;try{const data=await apiGet(`/messages/conversations/${id}?perPage=100`);setConversation(data.conversation||null);setMessages(data.messages||[]);setError("");}catch(e){setError(e.message||"Conversation unavailable.");}};
	useEffect(()=>{loadThreads();const id=setInterval(loadThreads,12000);return()=>clearInterval(id);},[]);
	useEffect(()=>{if(!selected)return;setParams({conversation:selected},{replace:true});loadConversation(selected);const id=setInterval(()=>loadConversation(selected),8000);return()=>clearInterval(id);},[selected]);
	useEffect(()=>{const vendorOrder=params.get("vendorOrder");if(!vendorOrder)return;let live=true;apiPost("/messages/conversations",{kind:"order",vendorOrderId:vendorOrder}).then((row)=>{if(!live)return;setSelected(row.id);setParams({conversation:row.id},{replace:true});loadThreads();}).catch((e)=>live&&setError(e.message));return()=>{live=false;};},[]);
