...
## Milestone AK lifecycle requirements

Payment providers expose a lookup operation used for reconciliation. Lookup never substitutes for signed webhook settlement. Provider creation retries must reuse the same local payment public ID/idempotency key when the first response is ambiguous.

Shipping providers expose lookup and cancellation in addition to label creation and signed webhook verification. Shipment creation retries reuse the same shipment public ID and idempotency key. Generic HTTP courier calls send an `Idempotency-Key` header. Cancellation is only allowed before pickup; event history remains immutable.
