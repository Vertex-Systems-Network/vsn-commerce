...
class PublishMarketplaceNotification
{
    ...
    public function execute(
        ...
        $enabled = /** Inline callback for this operation. */ fn (string $channel): bool => $critical && in_array($channel, ['in_app', 'email'], true)
            ? true
            : $this->preferences->enabled($user, $category, $channel);

        if (! $enabled('in_app') && ! collect(['email', 'sms', 'push'])->contains(/** Inline callback for this operation. */ fn ($channel) => $enabled($channel))) {
            return null;
        }
                ...
                foreach (['email', 'sms', 'push'] as $channel) {
                    if ($enabled($channel)) {
                        NotificationDelivery::query()->firstOrCreate(
                            ['marketplace_notification_id' => $row->id, 'channel' => $channel],
        ...
        if ($notification->wasRecentlyCreated && $notification->in_app_visible) {
            event(new MarketplaceNotificationCreated($notification));
        }
