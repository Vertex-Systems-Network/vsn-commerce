...
class OpenConversation
{
    ...
    private function support(User $user):Conversation
    {
        ...
        return DB::transaction(/** Inline callback for this operation. */ function()use($user){$conversation=$this->findOrInsertConversation('support:user:'.$user->id,['kind'=>'support','subject'=>'VSN Support','created_by_user_id'=>$user->id,'status'=>'open']);$conversation->participants()->firstOrCreate(['user_id'=>$user->id],['participant_role'=>'customer','joined_at'=>now()]);return $conversation->fresh('participants.user');},3);
    }
