...
final class SubmitKycVerification
{
    ...
    public function execute(User $user,KycVerificationType $type,array $data):KycVerification
    {
        ...
        $verification=DB::transaction(/** Inline callback for this operation. */ function()use($user,$type,$data,$provider){
            ...
            $disk=(string)config('vsn.kyc.document_disk','local');$dir='kyc/'.$user->id.'/'.Str::uuid();$store=/** Inline callback for this operation. */ fn(?UploadedFile $file,string $name)=>$file?$file->storeAs($dir,$name.'.'.$file->extension(),$disk):null;$number=trim((string)($data['document_number']??''));$canonicalDigits=$number?preg_replace('/\D+/','',$number):'';
