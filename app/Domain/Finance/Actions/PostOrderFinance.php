...
class PostOrderFinance
{
    ...
    public function execute(Order $order):void
    {
        DB::transaction(/** Inline callback for this operation. */ function()use($order):void{
            ...
            $entries=[];$cash=(int)$order->total_minor;
            if($cash>0)$entries[]=['account'=>$order->payment_status===PaymentStatus::Paid?FinanceAccounts::PAYMENT_CLEARING:FinanceAccounts::COD_RECEIVABLE,'direction'=>FinanceDirection::Debit->value,'amount'=>$cash];
            if((int)$order->coin_redemption_minor>0)$entries[]=['account'=>FinanceAccounts::COIN_LIABILITY,'direction'=>FinanceDirection::Debit->value,'amount'=>(int)$order->coin_redemption_minor];
            ...
            if($platformDiscount===0 && (int)$order->discount_minor>0 && (int)$order->seller_discount_minor===0){$platformDiscount=(int)$order->discount_minor;}
            if($platformDiscount>0)$entries[]=['account'=>FinanceAccounts::COUPON_SUBSIDY,'direction'=>FinanceDirection::Debit->value,'amount'=>$platformDiscount];
            $commission=0;$platformTax=0;
            foreach($order->vendorOrders as $vo){
                ...
                if(!$vo->finance_posted_at)$vo->update(['finance_posted_at'=>now()]);
            }
            if($platformTax>0)$entries[]=['account'=>FinanceAccounts::SALES_TAX_PAYABLE,'direction'=>FinanceDirection::Credit->value,'amount'=>$platformTax];
            if($commission>0)$entries[]=['account'=>FinanceAccounts::PLATFORM_COMMISSION,'direction'=>FinanceDirection::Credit->value,'amount'=>$commission];
