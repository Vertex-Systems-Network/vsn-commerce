...
class MarketplaceAnalyticsService
{
    ...
    public function range(array $filters): array
    {
        ...
        if ($from->diffInDays($to) > $maxDays) {
            $from = $to->subDays($maxDays - 1)->startOfDay();
        }
    ...
    private function dailyTrend($orders, $refunds, string $timezone): array
    {
        $rows = [];
        foreach ($orders as $o) {
            $d = $o->placed_at->timezone($timezone)->toDateString();
            $rows[$d] ??= ['date' => $d, 'orders' => 0, 'paidOrderValueMinor' => 0, 'refundMinor' => 0];
            $rows[$d]['orders']++;
            $rows[$d]['paidOrderValueMinor'] += (int) $o->subtotal_minor + (int) $o->shipping_minor - (int) $o->discount_minor + (int) $o->tax_added_minor;
        }
        $orderMap = $orders->keyBy('id');
        foreach ($refunds as $r) {
            $order = $orderMap->get($r->order_id);
            $date = ($r->processed_at ?? $order?->placed_at)?->timezone($timezone)?->toDateString();
            if (! $date) {
                continue;
            }$rows[$date] ??= ['date' => $date, 'orders' => 0, 'paidOrderValueMinor' => 0, 'refundMinor' => 0];
            $rows[$date]['refundMinor'] += (int) $r->amount_minor;
        }
        ksort($rows);
    ...
    private function promotionBreakdown(CarbonImmutable $from, CarbonImmutable $to, string $currency): array
    {
        ...
        return $rows->groupBy('promotion_id')->map(/** Inline callback for this operation. */ function ($group) {
            $discount = (int) $group->sum('discount_minor');
            $platform = (int) $group->sum('platform_funded_minor');
            $seller = (int) $group->sum('seller_funded_minor');
            $orderIds = $group->pluck('order_id')->filter()->unique();
            $value = (int) Order::query()->whereIn('id', $orderIds)->sum(DB::raw('subtotal_minor + shipping_minor - discount_minor + tax_added_minor'));

            return ['promotionId' => $group->first()->promotion?->public_id, 'name' => $group->first()->promotion?->name ?? 'Promotion', 'orders' => $orderIds->count(), 'discountMinor' => $discount, 'platformFundedMinor' => $platform, 'sellerFundedMinor' => $seller, 'attributedOrderValueMinor' => $value, 'returnOnDiscountBps' => $discount ? intdiv($value * 10000, $discount) : null];
        })->sortByDesc('attributedOrderValueMinor')->values()->take(20)->all();
    }
    ...
    private function customerCohorts(CarbonImmutable $from, CarbonImmutable $to, string $currency): array
    {
        ...
        $userIds = $first->pluck('user_id');
        $all = Order::query()->whereIn('user_id', $userIds)->whereIn('payment_status', [PaymentStatus::Paid->value, PaymentStatus::PartiallyRefunded->value, PaymentStatus::Refunded->value])->where('currency', $currency)->where('placed_at', '<=', $to->addDays(30))->get(['user_id', 'subtotal_minor', 'shipping_minor', 'discount_minor', 'tax_added_minor', 'placed_at']);
        $byUser = $all->groupBy('user_id');
        $cohorts = [];
        foreach ($first as $row) {
            $firstAt = CarbonImmutable::parse($row->first_order_at);
            $key = $firstAt->format('Y-m');
            $cohorts[$key] ??= ['cohort' => $key, 'newCustomers' => 0, 'repeatWithin30Days' => 0, 'orderValueMinor' => 0];
            $cohorts[$key]['newCustomers']++;
            $orders = $byUser->get($row->user_id, collect());
            $repeat = $orders->filter(/** Inline callback for this operation. */ fn ($o) => CarbonImmutable::parse($o->placed_at)->gt($firstAt) && CarbonImmutable::parse($o->placed_at)->lte($firstAt->addDays(30)))->isNotEmpty();
            if ($repeat) {
                $cohorts[$key]['repeatWithin30Days']++;
            }$cohorts[$key]['orderValueMinor'] += (int) $orders->filter(/** Inline callback for this operation. */ fn ($o) => CarbonImmutable::parse($o->placed_at)->gte($firstAt) && CarbonImmutable::parse($o->placed_at)->lte($firstAt->addDays(30)))->sum(/** Inline callback for this operation. */ fn ($o) => (int) $o->subtotal_minor + (int) $o->shipping_minor - (int) $o->discount_minor + (int) $o->tax_added_minor);
        }
        foreach ($cohorts as &$c) {
            $c['repeatWithin30DaysBps'] = $c['newCustomers'] ? intdiv($c['repeatWithin30Days'] * 10000, $c['newCustomers']) : 0;
        }unset($c);
        ksort($cohorts);
