...
class CatalogMutationService
{
    ...
    public function update(Product $product,User $actor,array $data,bool $admin=false):Product
    {
        ...
        $result=DB::transaction(/** Inline callback for this operation. */ function()use($product,$actor,$data,$admin,$oldPrice){
            $fields=[];$map=['categoryId'=>'category_id','sku'=>'sku','name'=>'name','shortDescription'=>'short_description','description'=>'description','currency'=>'currency','basePriceMinor'=>'base_price_minor','compareAtPriceMinor'=>'compare_at_price_minor','installmentEnabled'=>'installment_enabled','gameEnabled'=>'game_enabled','priceIncludesTax'=>'price_includes_tax','metadata'=>'metadata'];foreach($map as $in=>$db)if(array_key_exists($in,$data))$fields[$db]=$data[$in];if(array_key_exists('taxClassId',$data))$fields['tax_class_id']=!empty($data['taxClassId'])?\App\Models\TaxClass::query()->where('public_id',$data['taxClassId'])->value('id'):null;
            if(array_key_exists('slug',$data))$fields['slug']=$this->uniqueSlug($data['slug']?:($data['name']??$product->name),$product->id);
            if($admin&&array_key_exists('status',$data))$fields['status']=$data['status'];elseif(!$admin&&$product->status===ProductStatus::Published)$fields['status']=ProductStatus::PendingReview->value;
    ...
    public function submit(Product $product):Product{abort_unless(in_array($product->status,[ProductStatus::Draft,ProductStatus::PendingReview],true),422,'Only draft or pending-review products can be submitted.');$product->update(['status'=>ProductStatus::PendingReview]);$this->cache->bump();return $product->fresh(['vendor','category','taxClass','images.mediaAsset','variants.inventories']);}
    /** Updates stock. */
    ...
    {
        abort_unless($onHand>=0,422,'Stock cannot be negative.');$inventory=DB::transaction(/** Inline callback for this operation. */ function()use($variant,$actor,$onHand,$safetyStock,$reason){$warehouse=Warehouse::query()->where('is_active',true)->orderBy('id')->first()??Warehouse::query()->create(['code'=>'MAIN','name'=>'Main Warehouse','is_active'=>true]);$row=Inventory::query()->firstOrCreate(['warehouse_id'=>$warehouse->id,'product_variant_id'=>$variant->id],['on_hand'=>0,'reserved'=>0,'safety_stock'=>0]);$row=Inventory::query()->whereKey($row->id)->lockForUpdate()->first();$old=$row->on_hand;$changes=['on_hand'=>$onHand];if($safetyStock!==null)$changes['safety_stock']=max(0,$safetyStock);$row->update($changes);$delta=$onHand-$old;if($delta!==0)InventoryMovement::create(['inventory_id'=>$row->id,'type'=>InventoryMovementType::Adjustment,'on_hand_delta'=>$delta,'reserved_delta'=>0,'reference_type'=>'catalog_adjustment','reference_id'=>(string)$actor->id,'metadata'=>['reason'=>$reason,'actor_user_id'=>$actor->id]]);return $row->fresh();},3);$this->alerts->execute($variant->product);$this->cache->bump();return $inventory;
    }
    ...
    private function syncVariants(Product $product,User $actor,array $rows,bool $admin):void
    {
        ...
        if(count($provided)!==count(array_unique(array_map('strtolower',$provided)))) throw ValidationException::withMessages(['variants'=>['Variant SKUs must be unique within the product payload.']]);
        foreach($rows as $row){$sku=trim((string)($row['sku']??''));if($sku==='')continue;$existing=ProductVariant::query()->where('sku',$sku)->when(!empty($row['id']),/** Inline callback for this operation. */ fn($q)=>$q->where('id','!=',(int)$row['id']))->exists();if($existing)throw ValidationException::withMessages(['variants'=>["Variant SKU {$sku} is already in use."]]);}
        $seen=[];foreach($rows as $i=>$row){$id=$row['id']??null;$variant=$id?ProductVariant::query()->where('product_id',$product->id)->whereKey($id)->first():null;$old=$variant?->price_minor;
            ...
            if($variant)$variant->update($payload);else $variant=$product->variants()->create($payload);$seen[]=$variant->id;
            if($old!==$variant->price_minor||!$id)$this->recordPrice($product,$variant,$actor,$admin?'admin':'seller');
            if(array_key_exists('stock',$row))$this->setStock($variant,$actor,(int)$row['stock'],isset($row['safetyStock'])?(int)$row['safetyStock']:null,'variant_sync');
        }
    ...
    { $base=Str::slug($value)?:'product';$slug=$base;$n=2;while(Product::query()->withTrashed()->where('slug',$slug)->when($exceptId,/** Inline callback for this operation. */ fn($q)=>$q->where('id','!=',$exceptId))->exists())$slug=$base.'-'.$n++;return $slug; }
}
