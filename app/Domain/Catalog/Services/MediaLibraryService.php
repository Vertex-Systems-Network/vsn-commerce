...
class MediaLibraryService
{
    ...
    public function __construct(
        private readonly SecureUploadInspector $uploads,
    ...
    public function upload(User $actor, UploadedFile $file, ?Vendor $vendor = null, ?string $alt = null): MediaLibraryAsset
    {
        ...
        $existing = MediaLibraryAsset::query()
        ...
        $extension = match ($inspected['mime']) {
            'image/png' => 'png',
            'image/webp' => 'webp',
            default => 'jpg',
        };
        ...
        try {
            return DB::transaction(
                /** Persists the reusable media row atomically. */
                function () use ($actor, $file, $vendor, $alt, $scopeKey, $disk, $path, $inspected, $existing): MediaLibraryAsset {
                    $payload = [
                        'vendor_id' => $vendor?->id,
                        'uploaded_by_user_id' => $actor->id,
                        'scope_key' => $scopeKey,
    ...
    public function attach(Product $product, MediaLibraryAsset $library, User $actor, ?string $alt = null): ProductMediaAsset
    {
        ...
        $existing = ProductMediaAsset::query()
            ->where('product_id', $product->id)
            ->where('sha256', $library->sha256)
            ->first();
        ...
        $result = DB::transaction(
            /** Creates the product-specific media reference while reusing the library object. */
            function () use ($product, $library, $actor, $alt, $existing): ProductMediaAsset {
                ...
                $payload = [
                    ...
                    'disk' => $library->disk,
                    'path' => $library->path,
                    'original_name' => $library->original_name,
                    'alt_text' => $cleanAlt,
                    'mime_type' => $library->mime_type,
                    'byte_size' => $library->byte_size,
                    ...
                    'metadata' => [
                        ...
                        'media_library_asset_id' => $library->public_id,
                    ],
                    'sort_order' => $sort,
                ];
                ...
                ProductImage::create([
                    ...
                    'url' => Storage::disk($library->disk)->url($library->path),
                    'source' => 'managed',
                    'alt_text' => $cleanAlt,
                    'sort_order' => $sort,
                ]);
    ...
    {
        $usedByProduct = ProductMediaAsset::query()
            ->where('status', 'active')
            ->where('metadata->media_library_asset_id', $asset->public_id)
            ->exists();
        abort_if($usedByProduct, 422, 'This media item is currently used by a product. Remove it from products before archiving it.');

        $usedByStorefront = Vendor::query()
            ->where('metadata->logoMediaAssetId', $asset->public_id)
            ->exists();
