...
class ReviewController extends Controller
{
    ...
    public function store(SubmitReviewRequest $request, SubmitVerifiedReview $action, SecureUploadInspector $uploads): ReviewResource|JsonResponse
    {
        $data = $request->validated();
        foreach ($request->file('images', []) as $image) $uploads->inspect($image, ['image/jpeg','image/png','image/webp'], 5_242_880, true);
        try {
            $review = $action->execute($request->user(), (int)$data['orderItemId'], (int)$data['rating'], $data['text'], $request->file('images', []));
    ...
    public function product(Product $product, Request $request): JsonResponse
    {
        ...
        $reviews = Review::query()->where('product_id',$product->id)->where('status',ReviewStatus::Approved->value)
            ->with(['user','order','orderItem','product.images','product.vendor','images','sellerReplier'])->latest('submitted_at')->paginate(min(50,max(1,(int)$request->integer('perPage',10))));
        return response()->json(['data'=>[
            'items'=>ReviewResource::collection($reviews->getCollection())->resolve($request),
            'meta'=>['currentPage'=>$reviews->currentPage(),'lastPage'=>$reviews->lastPage(),'total'=>$reviews->total()],
        ]]);
    }
}
