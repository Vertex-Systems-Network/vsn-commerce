...
class SellerCatalogController extends Controller
{
    ...
    public function index(Request $request): JsonResponse
    {
        ...
        $q = Product::query()->where('vendor_id', $vendor->id)->with(['vendor', 'category', 'images.mediaAsset', 'variants.inventories'])->when($request->string('status')->toString(), /** Inline callback for this operation. */ fn ($x, $s) => $x->where('status', $s))->latest();
        $rows = $q->paginate(50);

        return response()->json(['data' => ['items' => CatalogManagementProductResource::collection($rows->getCollection())->resolve($request), 'meta' => ['total' => $rows->total(), 'currentPage' => $rows->currentPage(), 'lastPage' => $rows->lastPage()], 'categories' => $this->categories()]]);
    }
    ...
    {
        $vendor = $this->vendors->forUser($request->user());
        $product = $service->create($vendor, $request->user(), $this->validated($request), false);

        return new CatalogManagementProductResource($product);
    }
    ...
    public function update(Request $request, Product $product, CatalogMutationService $service): CatalogManagementProductResource
    {
        $vendor = $this->vendors->forUser($request->user());
        abort_unless($product->vendor_id === $vendor->id, 404);
        abort_if(in_array($product->status->value, ['suspended', 'archived'], true), 422, 'This product is locked by marketplace review.');

        return new CatalogManagementProductResource($service->update($product, $request->user(), $this->validated($request, true), false));
    }

    /** Handles submit for the seller catalog controller workflow. */
    public function submit(Request $request, Product $product, CatalogMutationService $service): CatalogManagementProductResource
    {
        $vendor = $this->vendors->forUser($request->user());
        abort_unless($product->vendor_id === $vendor->id, 404);

        return new CatalogManagementProductResource($service->submit($product));
    }
    ...
    public function stock(Request $request, ProductVariant $variant, CatalogMutationService $service): JsonResponse
    {
        ...
        $row = $service->setStock($variant, $request->user(), (int) $d['onHand'], isset($d['safetyStock']) ? (int) $d['safetyStock'] : null, $d['reason'] ?? 'seller_adjustment');
    ...
    {
        return Category::query()->where('is_active', true)->orderBy('sort_order')->orderBy('name')->get(['id', 'name', 'slug'])->toArray();
    }

    /** Validates seller catalog fields; product images are managed only through media endpoints. */
    private function validated(Request $request, bool $partial = false): array
    {
        $req = $partial ? 'sometimes' : 'required';

        return $request->validate(['name' => [$req, 'string', 'max:190'], 'slug' => ['sometimes', 'nullable', 'string', 'max:190'], 'sku' => ['sometimes', 'nullable', 'string', 'max:120', Rule::unique('products', 'sku')->ignore($request->route('product')?->id)], 'categoryId' => [$req, 'integer', 'exists:categories,id'], 'shortDescription' => ['sometimes', 'nullable', 'string', 'max:1000'], 'description' => ['sometimes', 'nullable', 'string', 'max:20000'], 'currency' => ['sometimes', 'string', 'size:3'], 'basePriceMinor' => [$req, 'integer', 'min:1'], 'compareAtPriceMinor' => ['sometimes', 'nullable', 'integer', 'min:1'], 'installmentEnabled' => ['sometimes', 'boolean'], 'gameEnabled' => ['sometimes', 'boolean'], 'taxClassId' => ['sometimes', 'nullable', 'string', 'exists:tax_classes,public_id'], 'priceIncludesTax' => ['sometimes', 'nullable', 'boolean'], 'images' => ['prohibited'], 'variants' => [$req, 'array', 'min:1', 'max:100'], 'variants.*.id' => ['sometimes', 'integer'], 'variants.*.sku' => ['sometimes', 'nullable', 'string', 'max:120'], 'variants.*.name' => ['required_with:variants', 'string', 'max:160'], 'variants.*.options' => ['sometimes', 'array'], 'variants.*.priceMinor' => ['sometimes', 'nullable', 'integer', 'min:1'], 'variants.*.compareAtPriceMinor' => ['sometimes', 'nullable', 'integer', 'min:1'], 'variants.*.isDefault' => ['sometimes', 'boolean'], 'variants.*.isActive' => ['sometimes', 'boolean'], 'variants.*.stock' => ['sometimes', 'integer', 'min:0'], 'variants.*.safetyStock' => ['sometimes', 'integer', 'min:0']]);
    }
}
