...
class MessageController extends Controller
{
    ...
    public function index(Request $request, UnreadMessageCounter $counter): JsonResponse
    {
        ...
        $query = Conversation::query()->with([
            ...
            'vendor',
            'participants.user',
            'messages' => /** Inline callback for this operation. */ fn ($q) => $q->with(['sender', 'attachments', 'conversation.vendor'])->latest('id')->limit(60),
        ]);

        if (in_array($role, [UserRole::Support->value, UserRole::Admin->value, UserRole::SuperAdmin->value], true)) {
            $query->where(/** Inline callback for this operation. */ fn ($q) => $q
                ->whereHas('participants', /** Inline callback for this operation. */ fn ($participant) => $participant->where('user_id', $user->id))
                ->orWhere('kind', 'support'));
        } else {
            $query->whereHas('participants', /** Inline callback for this operation. */ fn ($q) => $q->where('user_id', $user->id));
        }
        ...
        foreach ($rows as $conversation) {
            ...
            $unread = $conversation->messages()->where('sender_user_id', '!=', $user->id);
            if ($participant?->last_read_at) {
                $unread->where('created_at', '>', $participant->last_read_at);
            }
            $conversation->setAttribute('unread_count', $unread->count());
        }

        return response()->json([
            'data' => [
                'items' => ConversationResource::collection($rows)->resolve($request),
                'unreadCount' => $counter->forUser($user),
            ],
        ]);
    }

    /** Handles the store request for this resource. */
    public function store(Request $request, OpenConversation $action): JsonResponse
    {
        $data = $request->validate([
            'kind' => 'required|in:order,support',
            'vendorOrderId' => 'nullable|string|max:190',
        ]);

        try {
            $conversation = $action->execute($request->user(), $data['kind'], $data['vendorOrderId'] ?? null);
    ...
    public function show(Request $request, Conversation $conversation, ConversationAccess $access): JsonResponse
    {
        ...
        $conversation->participants()->firstOrCreate(
            ...
            ['participant_role' => 'support', 'joined_at' => now()],
        );
        ...
        $messages = ConversationMessage::query()
            ...
            ->with(['sender', 'attachments', 'conversation.vendor'])
            ->latest('id')
            ->paginate(min(100, max(1, (int) $request->input('perPage', 50))));
        $ordered = $messages->getCollection()->sortBy('id')->values();
        ...
        return response()->json([
            'data' => [
                ...
                'messages' => ConversationMessageResource::collection($ordered)->resolve($request),
                'meta' => [
    ...
    public function send(
        ...
        $data = $request->validate([
            ...
            'clientId' => 'required|string|max:190',
            'attachments' => 'nullable|array|max:4',
            'attachments.*' => 'file|max:10240|mimetypes:image/jpeg,image/png,image/webp,application/pdf',
        ]);

        foreach ($request->file('attachments', []) as $file) {
            $uploads->inspect($file, ['image/jpeg', 'image/png', 'image/webp', 'application/pdf'], 10_485_760, true);
        }
        ...
        return response()->json([
            'data' => (new ConversationMessageResource($message))->resolve($request),
        ]);
    }

    /** Handles read for the message controller workflow. */
    public function read(Request $request, Conversation $conversation, ConversationAccess $access): JsonResponse
    {
        $access->assert($request->user(), $conversation);
        $conversation->participants()->firstOrCreate(
            ['user_id' => $request->user()->id],
    ...
    public function attachment(Request $request, MessageAttachment $attachment, ConversationAccess $access): mixed
    {
        $attachment->load('message.conversation');
        $access->assert($request->user(), $attachment->message->conversation);
    ...
    private function error(MessagingException $exception): JsonResponse
    {
        return response()->json([
