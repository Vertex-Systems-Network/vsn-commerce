...
class NotificationController extends Controller
{
    ...
    {
        $query = MarketplaceNotification::query()
        ...
        return response()->json(['data' => [
            'items' => MarketplaceNotificationResource::collection($rows->getCollection())->resolve($request),
            'meta' => [
                'total' => $rows->total(),
                'currentPage' => $rows->currentPage(),
                'lastPage' => $rows->lastPage(),
                'unreadCount' => MarketplaceNotification::query()
                    ->where('user_id', $request->user()->id)
                    ->where('in_app_visible', true)
                    ->whereNull('read_at')
                    ->count(),
    ...
    public function read(Request $request, MarketplaceNotification $notification): JsonResponse
    {
        ...
        return response()->json([
            'data' => (new MarketplaceNotificationResource($notification->fresh()))->resolve($request),
        ]);
    }
    ...
    {
        $count = MarketplaceNotification::query()
            ->where('user_id', $request->user()->id)
            ->where('in_app_visible', true)
            ->whereNull('read_at')
            ->update(['read_at' => now()]);
    ...
    {
        return response()->json(['data' => [
            'preferences' => $service->matrix($request->user()),
