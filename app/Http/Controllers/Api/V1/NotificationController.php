...
class NotificationController extends Controller
{
    ...
    {
        $query = MarketplaceNotification::query()->where('user_id', $request->user()->id)->where('in_app_visible', true)->latest();
    ...
    public function read(Request $request, MarketplaceNotification $notification): MarketplaceNotificationResource
    {
...
return new MarketplaceNotificationResource($notification->fresh());
    }

    /** Handles read all for the notification controller workflow. */
    public function readAll(Request $request): JsonResponse
    {
        $count = MarketplaceNotification::query()->where('user_id', $request->user()->id)->where('in_app_visible', true)->whereNull('read_at')->update(['read_at' => now()]);

        return response()->json(['data' => ['ok' => true, 'updated' => $count, 'unreadCount' => 0]]);
    }

    /** Handles preferences for the notification controller workflow. */
    public function preferences(Request $request, NotificationPreferenceService $service): JsonResponse
    {
        return response()->json(['data' => ['preferences' => $service->matrix($request->user()), 'categories' => NotificationPreferenceService::CATEGORIES, 'channels' => NotificationPreferenceService::CHANNELS]]);
    }
    ...
    public function updatePreferences(Request $request, NotificationPreferenceService $service): JsonResponse
    {
        ...
        return response()->json(['data' => ['preferences' => $service->update($request->user(), $data['preferences'])]]);
    }
}
