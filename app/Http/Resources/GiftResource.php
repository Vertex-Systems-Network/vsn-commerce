...
class GiftResource extends JsonResource
{
    ...
    public function toArray(Request $request): array
    {
        ...
        return [
            ...
            'product'=>[
                'id'=>$this->product?->id,'slug'=>$this->product?->slug,'name'=>$this->product?->name,
                'image'=>$this->product?->images?->first()?->url,'variant'=>$this->variant?->name,
            ],
            ...
            'paymentIntent'=>$viewerId === $this->sender_user_id && $this->checkoutSession?->relationLoaded('paymentIntents') && $this->checkoutSession->paymentIntents->isNotEmpty() ? (/** Inline callback for this operation. */ function () { $intent=$this->checkoutSession->paymentIntents->sortByDesc('id')->first(); return ['id'=>$intent->public_id,'status'=>$intent->status->value,'provider'=>$intent->provider,'amountMinor'=>$intent->amount_minor,'sandboxCanSimulate'=>$intent->provider==='sandbox' && (bool)config('vsn.payments.providers.sandbox.simulator_enabled') && !app()->isProduction()]; })() : null,
